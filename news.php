	...
	<body>
		<?php
			...
			//isset($_SESSION['allow']) {
				...
				ShowMenu();
				$sqlQuery = 'SELECT *
					FROM news' or die("Error in the consult.." . mysqli_error($conn));
				$result = $conn->query($sqlQuery);
				
				echo mysqli_error($conn);
				if(isset($_SESSION['allow'])) {
					echo "<button  type=\"button\" onclick=\"location.href='newsPage.php?Id='\"><img src=\"Pictures/plus.png\" width=\"20px\" height=\"20px\"></button>"; //knopka
				}
				...
				while($row = mysqli_fetch_array($result)){
					if ($counter >= 10*$numberResults and $counter < 10*($numberResults + 1)){
						...
						echo "<pre class=\"textStyle\"><h3 id=\"topic\">" .$row['Topic'] ."</h3>" .substr($row['Text'] ,0 ,$numberOfChars);
				...
				$max = $max / 10;
				for ($i = 0; $i < $max; $i++) {
					$displayStr = $displayStr ."<a href=\"news.php?numberResults=" .$i ."\" id=\"numeration\">" .($i + 1) ."</a>";
				}
